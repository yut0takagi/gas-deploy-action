...
## ロールバックの実 API 検証（2026-08-10）

検証用スクリプト（`gas-deploy-actions-spike`）に対して、連続する2つのバージョンとそれを指すデプロイを作り、ロールバックを実行した。

### 確認できたこと

| 項目 | 結果 |
|---|---|
| `projects.versions.get` が `versionNumber` / `description` / `createTime` を返す | OK |
| 存在しないバージョンは 404 | OK |
| `dry-run` は書き込まない | OK |
| デプロイが実際に古いバージョンを指すようになる | OK（v16 → v15 を観測） |
| **Web アプリ URL がロールバックで変わらない** | OK |
| **HEAD のソースは戻らない** | OK（`VERSION_B` のまま） |

「HEAD は戻らない」は警告として実装していた想定だが、実測でも確認できた。

### 重大な発見: デプロイの読み取りは write-after-read が保証されない

`deployments.update` の直後、読み取りは長時間にわたり古い値を返す。しかも**単調ですらない**。

`deployments.list` を1秒間隔でポーリングした実測（v8 → v7 に書き換え）:

```
+1325ms  list=v7  get=v8
+3828ms  list=v8  get=v8     ← 古い値に戻った
+6044ms  list=v7  get=v8
+8517ms  list=v7  get=v7
```

別の実行では、`list` が**30秒間ずっと古い値を返し続けた**（`14, 14, 14, ... , 14`）。複数のレプリカが別々の履歴地点を返していると考えられる。

`deployments.get`（単体取得）は同条件で数秒以内に収束し、`list` のような振動は観測されなかった。ただし**完全ではない**: 書き込み直後に単体取得を2秒間隔で2回読み、両方とも古い同一の値を返した実行があった。

### 実装への反映

- ロールバックの「現在のバージョン」の確定を `deployments.list` から `deployments.get` に変更した。一覧は「どのデプロイを対象にするか」の特定にのみ使う（デプロイの ID と @HEAD かどうかは反映遅延で入れ替わらない）
- 連続2回の読み取りが一致するまで待つ安定化を入れた。揺れが収まらず戻り先も未指定なら、推測せず失敗して `version-number` の明示を促す

### 残る制約（設計上、API 側では解消できない）

**デプロイ直後にロールバックすると、「1つ前」の計算が誤ることがある。** 上記の緩和策は確率を下げるだけで、2回の読み取りが両方とも同じ古いレプリカに当たれば通ってしまう（実測で発生）。障害対応で最もありがちな流れがこの窓に入るため、実用上は `version-number` を明示するのが確実。

`deployer.ts` がデプロイ上限の警告のために行う `listDeployments` も同じ影響を受ける。件数の警告が1回分遅れて出ることがあるが、助言的な警告なので実害は小さい。
