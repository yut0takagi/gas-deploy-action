...
★ が本番事故を防ぐ経路である。`type: webapp | addon` で `deploymentId` が未指定の場合、実行前に警告を出す。
...
| # | 内容 | 影響 | 検証タイミング |
|---|---|---|---|
| 1 | clasp v3 の `.clasprc.json` スキーマが v2 から変わっているか | 移行ストーリーの中核。両形式パースで吸収する前提 | v0.1 実装初期 |
| 2 | Web アプリのデプロイに `script.webapp.deploy` スコープが追加で必要か | 要求スコープが1つ増える | v0.1 実装初期 |
| 3 | `script.projects` が Google の審査対象スコープ（sensitive/restricted）か | **個人 Gmail 対応の難易度を左右する最大の不確定要素** | **v0.1 実装初日** |
| 4 | デプロイ数上限20の真偽と正確な単位 | 警告閾値の設定と v2.0 の自動回収機能の設計 | v0.1 実装中 |
| 5 | サブディレクトリを `name` に `/` 区切りで表現できるか | file-collector の実装方針 | v0.1 実装初期 |
| 6 | clasp v3 の `.clasprc.json` が複数ユーザーを保持しうるか | 保持しうる場合、パーサが「最初に見つかった認証情報」を黙って採用する。CI で意図しないアカウントとしてデプロイされる | **Task 5 のスパイクで併せて確認** |

| 7 | **`invalid_rapt` の原因と回避策** | clasp v3.3.0 + 個人 Gmail で、ログイン直後にもかかわらずトークン交換が `invalid_rapt` で失敗する事象を観測。恒常的なら**認証方式の前提が崩れる最優先の論点** | **最優先。切り分け手順は下記** |

**#7 の切り分け手順**:

1. **clasp 自身が同じ失敗をするか** — `npx @google/clasp list` などリフレッシュを伴う操作を実行する。同じく失敗するなら Google／アカウント側の問題で、本実装のバグではない。成功するなら本実装の認証情報の取り出し方に問題がある
2. **スパイクの `[7]` を `[2]` より前に実行する** — `.clasprc.json` の構造を見れば、パーサが意図しない候補を拾っていないか判定できる。現状は失敗する `[2]` が先にあるため診断情報が得られない（スクリプト側の設計ミス）
3. 結果に応じて、専用の OAuth クライアント（`clasp login --creds`）でトークンを発行し直す案を検討する

**#6 の補足**: 認証情報パーサは入れ子を再帰的に探索して最初に一致したものを返す。`{tokens: {alice: {...}, bob: {...}}}` のような複数ユーザー構成では alice が黙って選ばれる。ただし誤ったアカウントを選んだ場合は Apps Script API が 403/404 を返して**明示的に失敗する**（サイレントに誤ったスクリプトへ書き込むことはない）ため、v0.1 では許容する。v3 の実際のレイアウトが判明した時点で、`default` キーの優先や候補が複数ある場合のエラー化を検討する。
