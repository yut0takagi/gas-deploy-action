...
複数アクションを1リポジトリで提供する。各アクションをサブディレクトリに置くことで、`<owner>/<repo>/deploy@v1` の形で個別に参照でき、v1.0 で `refresh-token` を追加する際もリポジトリを増やさずに済む。

> 実際のリポジトリ名は `gas-deploy-action`（単数形）になった。v0.1 で提供するアクションが `deploy` の1つだけであるため。サブディレクトリ構成は将来の追加を見越して維持する。
