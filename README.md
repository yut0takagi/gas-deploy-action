...
- **`rollback` は実 API に対して未検証。** ユニットテストは通っているが、`projects.versions.get` の応答形と、`deployments.update` に過去の `versionNumber` を渡したときの実挙動（Web アプリ URL が維持されるか、反映までの遅延があるか）は実測していない。デプロイ側で `deployments.update` による URL 維持は確認済みなので同じ経路のはずだが、確認が済むまでは本番前に `dry-run` で戻り先を確かめること。
