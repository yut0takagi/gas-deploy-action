...
### ⚠️ Workspace の再認証ポリシーによる失効（同意画面とは別の問題）

前節の「7日で失効」とは**独立した、別の失効経路**である。同意画面を **本番** や **内部** にしても発生する。同意画面を直したのに1週間後にまた止まった、という場合はこちらを疑うこと。

Google Workspace アカウントに**再認証ポリシー**（reauthentication policy）が設定されていると、ポリシーの期限を過ぎた時点で保存済みの refresh token が失効する。エラーはトークンエンドポイントから次の形で返る。

```
invalid_grant - reauth related error (invalid_rapt)
```

本 Action はこのレスポンスを検出し、他の `invalid_grant`（トークンの取り消し、クロックずれ等）とは区別して、原因と対処を明示したメッセージで失敗する。

**復旧**（都度の対処。ポリシーの期限ごとに再発する）:
...
**恒久的な回避**: Workspace アカウントではなく**個人 Google アカウント**の認証情報を使う。Workspace で運用を続ける場合は、管理コンソールの再認証ポリシーの対象から外れた専用のデプロイ用アカウントを用意する。ポリシーが有効なままのアカウントは、無人 CI での利用に向かない。
...
### 書き込みの前に認証と権限をまとめて確認する

逐次実行では、3件目で権限切れに気づいたときには1・2件目が既に置き換わっている。これを避けるため、**デプロイを1件も始める前に**次を確認し、1つでも通らなければ何も書かずに終了する。

1. リフレッシュトークンの交換（失効していればここで、認証の問題として止まる）
2. 付与スコープの検証（不足していれば警告。トークンエンドポイントが `scope` を省略することがあるため、ここでは落とさない）
3. **対象の全 `script-id` の読み取り権限**（`projects.get`。1件でも読めなければ、その `script-id` を名指しして失敗する）

いずれも読み取りのみで、リモートのファイルには触れない。失敗時のメッセージには「まだ書き換えられていません」と明記するので、再実行してよいかの判断に迷わない。

単一プロジェクトモードでは 3 は行わない。`deploy` が書き込みの前に必ずリモートの内容を取得するため、そこで同じことが確認済みになる（追加で叩くとリクエストが1つ増えるだけになる）。
...
## ⚠️ `root-dir` の内容はリモートを完全に置換する

**差分適用ではない。** `root-dir` に無いファイルは、リモートから削除される。指定を誤ると、稼働中の Apps Script のコードが失われる。復旧には Apps Script のバージョン履歴を遡る必要がある。

事故が起きる典型的な経路は3つある。

- `root-dir` の指定ミス（ビルド前のディレクトリ、空のディレクトリ、階層のずれ）
- ビルドを伴う構成で、ビルドが失敗したまま次のステップに進んだ
- `.claspignore` の設定ミスで、意図せず対象から外れたファイルがある

### 既定で止まる範囲

**リモートの半数以上（かつ2件以上）が削除される場合、書き込みを行わずに失敗する。**

```
リモートの 5 件のうち 3 件が削除されるため、デプロイを中止しました

次の手順を確認してください:
  1. 削除されるファイル: Config.gs, Legacy.gs, utils/Helper.gs
  2. root-dir の指定が正しいか確認してください（誤ったディレクトリを指すと、
     リモートのファイルがほぼすべて削除されます）
  ...
  5. この削除が意図したものであれば allow-delete: true を指定してください
```
...
```yaml
      - uses: yut0takagi/gas-deploy-action/deploy@v0.2.0
        with:
          ...
          script-id: ${{ secrets.GAS_SCRIPT_ID }}
          root-dir: dist
          allow-delete: true    # 削除を伴うデプロイを許可する
```

**「1件でも削除なら止める」にはしていない。** ファイルを1つ消すたびにフラグを要求すると、やがて `allow-delete: true` を恒久的に置きっぱなしにすることになり、本当に止めたい事故のときにも素通りする。止める対象を事故の規模に絞ることで、フラグを立てる行為そのものに意味を残している。裏を返せば、**閾値未満の削除は止まらない。**

### dry-run では止めずに見せる

`dry-run: true` は `allow-delete` に関わらず失敗しない。書き込まずに削除内容を確認し、それから `allow-delete` を判断するための経路だからである。

削除はジョブサマリと PR コメントで、追加・変更とは独立した警告ブロックとして表示される。

> [!WARNING]
> **以下の 3 ファイルがリモートから削除されます**
>
> - `Config.gs`
> - `Legacy.gs`
> - `utils/Helper.gs`

`comment-on-pr: true` を併用すると、この警告が PR に出るのでレビューで気付ける。
...
| `root-dir` | — | `.` | アップロードするファイルのルートディレクトリ（単一プロジェクトモードのみ）。**この内容がリモートを完全に置換する**（下記の警告を参照） |
...
| `allow-delete` | — | `false` | 事故の規模の削除（リモートの半数以上かつ2件以上）を許可する。既定では書き込まずに失敗する |
...
- **Workspace の再認証ポリシー配下では refresh token が定期的に失効する。** 同意画面を **本番** / **内部** にしても回避できない、OAuth 同意画面のステータスとは独立した失効経路。`invalid_grant` / `invalid_rapt` で CI が定期的に落ちることになり、その都度 `clasp login` からのシークレット更新が必要になる。恒久的に避けるには個人 Google アカウント、またはポリシー対象外の専用アカウントを使うしかなく、これは本 Action ではなく Google Workspace 側の制約。詳細は[前述の該当節](#️-workspace-の再認証ポリシーによる失効同意画面とは別の問題)を参照。
...
- **`updateContent` はローカルディレクトリの内容で全ファイルを置き換える。** ローカルが真実になるので、`root-dir` の指定を誤ると稼働中のスクリプトから大半のファイルが消える。事故の規模の削除は既定で停止するが（[前述の該当節](#️-root-dir-の内容はリモートを完全に置換する)）、閾値未満の削除は止まらない。`root-dir` は必ず確認すること。
